...
#include "network/quic/connection.h"
#include "log.h"
...
void ConnectionQuic::putEvent(ConnectionEventPtr e)
{
	assert(e->type != CONNEVENT_NONE); // Pre-condition
	m_event_queue.push_back(e);
}

void ConnectionQuic::processCommand(ConnectionCommandPtr c)
{
	switch (c->type) {
	case CONNCMD_NONE:
...
// host
void ConnectionQuic::serve(Address bind_address)
{
...
// peer
void ConnectionQuic::connect(Address address)
{
...
void ConnectionQuic::disconnect()
{
	// MutexAutoLock peerlock(m_peers_mutex);
...
void ConnectionQuic::sendToAll(u8 channelnum, SharedBuffer<u8> data, bool reliable)
{
...
ENetPeer *ConnectionQuic::getPeer(u16 peer_id)
{
...
bool ConnectionQuic::deletePeer(u16 peer_id, bool timeout)
{
	// MutexAutoLock peerlock(m_peers_mutex);
	if (m_peers.find(peer_id) == m_peers.end())
		return false;
...
*/
size_t ConnectionQuic::events_size()
{
	return m_event_queue.size();
}

ConnectionEventPtr ConnectionQuic::waitEvent(u32 timeout_ms)
{
...
void ConnectionQuic::putCommand(ConnectionCommandPtr c)
{
...
void ConnectionQuic::Serve(Address bind_addr)
{
...
bool ConnectionQuic::ReceiveTimeoutMs(NetworkPacket *pkt, u32 timeout)
{
	for (;;) {
...
/*
bool ConnectionQuic::TryReceive(NetworkPacket *pkt)
{
...
Address ConnectionQuic::GetPeerAddress(session_t peer_id)
{
...
void ConnectionQuic::PrintInfo(std::ostream &out)
{
	out << getDesc() << ": ";
}

void ConnectionQuic::PrintInfo()
{
	PrintInfo(dout_con);
}

std::string ConnectionQuic::getDesc()
{
	return "";
	// return std::string("con(")+itos(m_socket.GetHandle())+"/"+itos(m_peer_id)+")";
}

float ConnectionQuic::getPeerStat(u16 peer_id, con::rtt_stat_type type)
{
	return 0;
}

float ConnectionQuic::getLocalStat(con::rate_stat_type type)
{
	return 0;
}

void ConnectionQuic::DisconnectPeer(u16 peer_id)
{
