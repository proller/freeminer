		...
		f32 camera_fov, f32 range, f32 *distance_ptr, int block_scale)
{
	...
	v3f blockpos(
			((float)blockpos_nodes.X + block_scale * MAP_BLOCKSIZE/2) * BS,
			((float)blockpos_nodes.Y + block_scale * MAP_BLOCKSIZE/2) * BS,
			((float)blockpos_nodes.Z + block_scale * MAP_BLOCKSIZE/2) * BS
	);
...
*/	
	...
	// adjusted position
	f32 adjdist = (block_scale * BLOCK_MAX_RADIUS) / cos((M_PI - camera_fov) / 2);
